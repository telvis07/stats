...
Use `dbinom` to get the probability mass function (PMF). The PMF shows the probability that a random variable *equals* some discrete value.

```{r dbinom, echo=FALSE, fig.width=6, fig.height=4, fig.align = "center"}
chap3.1 <- function(){
  ...
  g <- ggplot(df, aes(x=x, y=y)) +
    ...
    ylab("PMF") +
    theme(plot.title = element_text(hjust = 0.5)) +
    labs(title="Probability of x=[1,10] successes where X ~ Bern(0.2)")
  g
...
Use `pbinom` to get the cumulative distribution function (CDF). The CDF shows the probability that a random variable *is less than* some value.

```{r pbinom, echo=FALSE, fig.width=6, fig.height=4, fig.align = "center"}
chap3.2 <- function(){
